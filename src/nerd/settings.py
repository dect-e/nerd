...
try:
    if 'NERD_CONFIG_FILE' in os.environ:
        # Use read to avoid failing when file is not existing. This allows for auto creation in the given directory.
        config.read(os.environ.get('NERD_CONFIG_FILE', ''), encoding='utf-8')
    else:
        config.read(['nerd.cfg', '/etc/nerd/nerd.cfg'], encoding='utf-8')
...
if not len(config.sections()):
    ...
    conffile = os.path.abspath(os.environ.get('NERD_CONFIG_FILE', 'nerd.cfg'))
    logging.warn(
...
WSGI_APPLICATION = 'nerd.wsgi.application'
...
database_backend = os.environ.get('NERD_DB_ENGINE', config.get('database', 'engine', fallback='sqlite3'))
database_name = os.environ.get('NERD_DB_NAME', config.get('database', 'name', fallback='db.sqlite3'))
database_user = os.environ.get('NERD_DB_USER', config.get('database', 'user', fallback=''))
database_password = os.environ.get('NERD_DB_PASSWORD', config.get('database', 'password', fallback=''))
database_host = os.environ.get('NERD_DB_HOST', config.get('database', 'host', fallback=''))
database_port = os.environ.get('NERD_DB_PORT', config.get('database', 'port', fallback=''))
